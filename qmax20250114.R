...
lnfe_mean <- dat_qmax[, mean(log(feox), na.rm = TRUE)]
lnal_mean <- dat_qmax[, mean(log(alox), na.rm = TRUE)]

# 计算指定列的标准差
ph_sd <- dat_qmax[, sd(ph, na.rm = TRUE)]
lnsom_sd <- dat_qmax[, sd(log(som), na.rm = TRUE)]
lnclay_sd <- dat_qmax[, sd(log(clay), na.rm = TRUE)]
lnfe_sd <- dat_qmax[, sd(log(feox), na.rm = TRUE)]
lnal_sd <- dat_qmax[, sd(log(alox), na.rm = TRUE)]
...
dat_qmax[ , lnsom := log(((som)/1.724)*2*10)]     # 
...
# 保存结果为 CSV 文件
write.csv(dat_pred, file = "C:/Users/A/Desktop/Rds文件/20250125-res-2/qmax_predictions_noncalcareous_20250126.csv", row.names = FALSE)
## -----------------------------
## 8. 查看结果
## -----------------------------
head(dat_pred)
