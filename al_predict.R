...
## 1. 加载训练好的模型
## -----------------------------
full_model <- readRDS("C:/Users/A/Desktop/Rds文件/models/xgboost_alox_model_calcareous.rds") # al预测模型
...
#dat_pred <- dat_qmax[, .(ph, lnsom, lnclay, lnal, ln_totalp)]

# 使用 setnames() 批量重命名，必须与该列名保持一致
setnames(
 dat_pred,
 old = c("ph", "lnsom_scaled", "lnclay_scaled"),
 new = c("ph", "ln_som", "ln_clay")
)
# setnames(
#   dat_pred,
#   old = c("ph", "lnsom", "lnal", "lnclay", "ln_totalp"),
...
dat_pred <- as.data.table(cbind(dat_pred, lnal = predictions))

# dat_pred[ , lnal := lnal_scaled * lnal_sd + lnal_mean]

dat_pred[ , al := exp(lnal)] # * lnal_sd + lnal_mean)]
# 7. 保存结果为 CSV 文件
write.csv(dat_pred, file = "C:/Users/A/Desktop/Rds文件/20250125-res-2/lnal_predictions_cal_26.csv", row.names = FALSE)
