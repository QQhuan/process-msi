...
msi <- readImzML(data_path, memory = FALSE, verbose = TRUE)
...
# 查看校准后的数据概览
normalized_msi <- normalize(mse_calibrated, method = "tic")
...
mse_align <- peakAlign(mse_baselined)
# 数据平滑
mse_smoothed <- smooth(mse_align, method="gaussian", width=5) # 高斯平滑，宽度参数需根据数据调整
...
#print(mse_final)
#mse_final <- peakPick(mse_smoothed, method="diff", SNR=2)
mse_final <- process(mse_smoothed)
